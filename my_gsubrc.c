...
/* Changes by Jonathan:
 *   1.  receipts do not include full names
 *   2.  changed from foo@bar.com (Full Name) to Full Name <foo@bar.com>
 *       This required some number of changes, since print_id included an
 *       initial space, which had to be removed.  <sigh>
 */
...
static int print_id(struct gale_text var,struct gale_text dfl) {
	struct gale_text value = gale_var(var);
	int i = 0; /* number of IDs printed */
...
int gsubrc(void) {
	...
	if (0 != answer.l || 0 != presence.l) {
		...
		gale_print(stdout,0,G_(" "));
		had_id = print_id(G_("GALE_FROM"),G_("unverified"));

                /* only print name if unverified */
                if (!had_id && from_name.l) {
			gale_print(stdout,0,G_(" ("));
